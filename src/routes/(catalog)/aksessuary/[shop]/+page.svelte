...
<svelte:head>
	{#if shop}
		<title>{getShopName(shop)} – Аксессуары для кухни | Компания Новострой</title>
		<meta
			name="description"
			content={`${getShopName(shop)} — поставщик аксессуаров для кухни. ${shop.description || 'Рейлинги, органайзеры, корзины и системы хранения.'}`}
		/>
	{:else}
					...
					>
						...
						<div class="relative px-8 py-16 sm:px-12 sm:py-20">
							...
							<nav class="mb-6 flex items-center gap-2 text-sm text-slate-400">
								...
								<span>/</span>
								<a href="/aksessuary" class="hover:text-white transition-colors">Аксессуары</a>
								<span>/</span>
						...
						<div class="mt-8 rounded-2xl bg-white p-8 shadow-sm">
							...
							<div class="mt-4 flex flex-wrap gap-3">
								{#each shop.brands.filter((b) => b.is_active) as brand}
									...
									{:else}
										<span
											class="inline-flex items-center rounded-full bg-emerald-100 px-3 py-1.5 text-sm font-medium text-emerald-700"
										>
											{brand.value}
										</span>
									{/if}
							...
							<div class="mt-4 space-y-3">
								{#if shop.phone}
									<div class="flex items-center gap-3 text-slate-600">
										...
										<a
											href="tel:{shop.phone}"
											class="font-medium hover:text-emerald-600 transition-colors">{shop.phone}</a
										>
									</div>
								{/if}
								{#if shop.email}
									<div class="flex items-center gap-3 text-slate-600">
										...
										<a
											...
											class="font-medium hover:text-emerald-600 transition-colors">{shop.email}</a
										>
									</div>
								{/if}
...
{:else}
	...
	<div class="min-h-screen bg-slate-50 flex items-center justify-center">
		<div class="text-center px-4">
			...
			<a
				href="/aksessuary"
				class="mt-6 inline-flex items-center gap-2 rounded-lg bg-emerald-500 px-6 py-3 font-medium text-white transition-all hover:bg-emerald-600"
			>
				<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
					<path
						stroke-linecap="round"
