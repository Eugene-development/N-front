...
<svelte:head>
	{#if shop}
		<title>{getShopName(shop)} – Фурнитура | Компания Новострой</title>
		<meta
			name="description"
			content={`${getShopName(shop)} — поставщик мебельной фурнитуры. ${shop.description || 'Петли, направляющие, подъёмные механизмы и системы хранения.'}`}
		/>
	{:else}
					...
					>
						...
						<div class="relative px-8 py-16 sm:px-12 sm:py-20">
							...
							<nav class="mb-6 flex items-center gap-2 text-sm text-slate-400">
								...
								<span>/</span>
								<a href="/furnitura" class="hover:text-white transition-colors">Фурнитура</a>
								<span>/</span>
