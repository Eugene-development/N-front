		...
		<div class="mx-auto max-w-7xl px-4 py-3 sm:px-6 lg:px-8">
			<nav class="flex" aria-label="Breadcrumb">
				<ol class="flex items-center space-x-2 text-sm">
					...
					<li>
						<a href="/vacancy" class="text-slate-500 hover:text-sky-600 transition-colors">Вакансии</a>
					</li>
					<li class="flex items-center">
						<svg class="h-4 w-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
			...
			<div class="lg:col-span-2">
				...
				<div class="rounded-2xl bg-white p-8 shadow-sm">
					<div class="flex items-center gap-3">
						<span class="rounded-full {colors.bg} {colors.text} px-3 py-1 text-sm font-medium">{vacancy.department}</span>
						<span class="rounded-full bg-slate-100 px-3 py-1 text-sm font-medium text-slate-600">{vacancy.type}</span>
					</div>
				...
				<div class="mt-8 rounded-2xl bg-white p-8 shadow-sm">
					...
					<ul class="mt-4 space-y-3">
						{#each vacancy.requirements as item}
							<li class="flex items-start gap-3">
								<svg class="mt-1 h-5 w-5 shrink-0 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
				...
				<div class="mt-8 rounded-2xl bg-white p-8 shadow-sm">
					...
					<ul class="mt-4 space-y-3">
						{#each vacancy.conditions as item}
							<li class="flex items-start gap-3">
								<svg class="mt-1 h-5 w-5 shrink-0 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
								</svg>
								<span class="text-slate-600">{item}</span>
							</li>
				...
				<div class="sticky top-24 space-y-6">
					...
					<div class="rounded-2xl bg-white p-6 shadow-sm">
						...
						<div class="mt-4 space-y-3">
							<a href="mailto:hr@novostroy.ru" class="flex items-center gap-3 text-slate-600 hover:text-sky-600 transition-colors">
								<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
								</svg>
								hr@novostroy.ru
							</a>
							<a href="tel:+74951234567" class="flex items-center gap-3 text-slate-600 hover:text-sky-600 transition-colors">
								<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
					...
					<div class="rounded-2xl bg-white p-6 shadow-sm">
						...
						<div class="mt-4 space-y-3">
							{#if slug !== 'designer'}
								<a href="/vacancy/designer" class="block text-slate-600 hover:text-sky-600 transition-colors">Дизайнер мебели</a>
							{/if}
							{#if slug !== 'assembler'}
								<a href="/vacancy/assembler" class="block text-slate-600 hover:text-sky-600 transition-colors">Сборщик мебели</a>
							{/if}
							{#if slug !== 'driver'}
								<a href="/vacancy/driver" class="block text-slate-600 hover:text-sky-600 transition-colors">Водитель-экспедитор</a>
							{/if}
							{#if slug !== 'marketer'}
								<a href="/vacancy/marketer" class="block text-slate-600 hover:text-sky-600 transition-colors">Маркетолог</a>
							{/if}
						</div>
						<a href="/vacancy" class="mt-4 inline-flex items-center gap-1 text-sm font-medium text-sky-600 hover:text-sky-700">
							Все вакансии
...
<div class="min-h-screen bg-slate-50 flex items-center justify-center">
	<div class="text-center px-4">
		...
		<a href="/vacancy" class="mt-6 inline-flex items-center gap-2 rounded-xl bg-sky-500 px-6 py-3 font-medium text-white transition-all hover:bg-sky-600">
			<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
